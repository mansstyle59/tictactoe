...
const THEMES={
  light:{BG:"#fafaf9",CARD:"#ffffff",INK:"#0c0a09",MUTE:"#78716c",BORDER:"#e7e5e4",
         INPUT:"#ffffff",CHIP:"#f5f5f4",CHIP2:"#f0efee",RULE:"#f9f9f8"},
  dark: {BG:"#0f0f11",CARD:"#1c1c1f",INK:"#f4f4f5",MUTE:"#a1a1aa",BORDER:"#2e2e33",
         INPUT:"#27272a",CHIP:"#27272a",CHIP2:"#1c1c1f",RULE:"#18181b"},
};

// Simple module-level theme proxy - updated by App on each render
const _themeProxy={...THEMES.light};
const T=()=>_themeProxy;
function applyTheme(dark){
  const src=dark?THEMES.dark:THEMES.light;
  Object.assign(_themeProxy,src);
}

const colOf=p=>p==="X"?XC:OC;
const gradOf=p=>p==="X"?XG:OG;
const oppOf=p=>p==="X"?"O":"X";
// symOf proxy - updated by App
const _symOf_proxy={fn:(p)=>p};
const _symOf=(p)=>_symOf_proxy.fn(p);
...
function useTheme(dark){
  // Sync proxy on every render so all T() calls get current theme
  applyTheme(dark);
    ...
    <div style={{position:"fixed",inset:0,background:T().BG==="#0f0f11"?"rgba(0,0,0,0.82)":"rgba(12,10,9,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:160,padding:"20px",backdropFilter:"blur(8px)",animation:"fadeIn .15s ease"}}>
      <div style={{background:T().CARD,borderRadius:"20px",padding:"24px 20px",width:"100%",maxWidth:"320px",boxShadow:"0 20px 60px rgba(0,0,0,0.18)",animation:"slideUp .25s cubic-bezier(.34,1.56,.64,1)"}}>
        <div style={{textAlign:"center",marginBottom:"16px"}}>
          ...
          <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"16px",color:T().INK}}>Quelle pièce sacrifier ?</div>
          <div style={{fontSize:"12px",color:T().MUTE,marginTop:"4px"}}>Vous avez trop de pièces — choisissez laquelle disparaît</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
          {choices.map((idx,n)=>(
            <button key={idx} onClick={()=>{sfx.choose();onChoose(idx);}} style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:"12px",background:n===0?"#fff1f2":"#f9f9f8",border:`1.5px solid ${n===0?XC+"44":T().BORDER}`,borderRadius:"12px",cursor:"pointer",transition:"all .15s",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>
              ...
              <div style={{flex:1,textAlign:"left"}}>
                <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                  <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:"13px",color:T().INK}}>Case {idx+1}</span>
                ...
                <div style={{fontSize:"11px",color:T().MUTE}}>Position {n+1} dans la file</div>
              </div>
              <span style={{fontSize:"14px",color:T().MUTE}}>Sacrifier →</span>
            </button>
          ))}
        </div>
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"6px",marginBottom:"20px"}}>
          {data.breakdown.map((b,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 12px",background:T().RULE,borderRadius:"8px"}}>
              <span style={{fontSize:"13px",color:T().INK}}>{b.label}</span>
...
/* ── Small UI ────────────────────────────────────────────── */
function Toggle({value,onChange,color}){return(<div onClick={onChange} style={{width:"42px",height:"24px",borderRadius:"12px",background:value?(color||XC):T().BORDER,transition:"background .2s",position:"relative",flexShrink:0,cursor:"pointer",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}><div style={{position:"absolute",top:"3px",left:value?"21px":"3px",width:"18px",height:"18px",borderRadius:"50%",background:"#fff",transition:"left .2s",boxShadow:"0 1px 4px rgba(0,0,0,0.2)"}}/></div>);}
    ...
    <div style={{background:T().BG,minHeight:"100dvh",overflowY:"auto"}}>
      <div style={{maxWidth:"400px",margin:"0 auto",padding:"32px 20px 52px"}}>
        ...
        <div style={{display:"flex",gap:"6px",marginBottom:"20px",background:T().CHIP2,borderRadius:"14px",padding:"4px"}}>
          {[{id:"2p",label:"👥 2 Joueurs"},{id:"ai",label:"🤖 Vs IA"}].map(tb=><button key={tb.id} onClick={()=>{sfx.tap();setTab(tb.id);}} style={{flex:1,padding:"12px",fontSize:"14px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,background:tab===tb.id?T().CARD:"transparent",border:"none",color:tab===tb.id?T().INK:T().MUTE,cursor:"pointer",borderRadius:"11px",transition:"all .2s",boxShadow:tab===tb.id?"0 1px 8px rgba(0,0,0,0.1)":"none",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>{tb.label}</button>)}
        </div>
        ...
        <div style={{marginBottom:"20px"}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
            ...
            {tab==="ai"&&<div style={{background:T().CARD,borderRadius:"16px",padding:"12px 16px",border:`1.5px solid ${OC}22`}}><div style={{display:"flex",alignItems:"center",gap:"10px"}}><div style={{width:"44px",height:"44px",borderRadius:"50%",background:`${OC}15`,border:`2px solid ${OC}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"22px"}}>🤖</div><div><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"15px",color:T().INK}}>IA</div><div style={{fontSize:"12px",color:T().MUTE}}>{DIFFS.find(d=>d.id===aiLevel)?.label}</div></div></div></div>}
          </div>
        </div>

        {/* AI difficulty */}
        {tab==="ai"&&<div style={{marginBottom:"20px"}}><div style={{fontSize:"11px",color:T().MUTE,letterSpacing:"1px",marginBottom:"10px",textTransform:"uppercase",fontWeight:600}}>Difficulté</div><div style={{display:"flex",flexDirection:"column",gap:"6px"}}>{DIFFS.map(lv=>{const sel=aiLevel===lv.id;return(<button key={lv.id} onClick={()=>{sfx.tap();setAiLevel(lv.id);}} style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:"12px",background:sel?T().CARD:"transparent",border:`1.5px solid ${sel?lv.col:T().BORDER}`,borderRadius:"12px",cursor:"pointer",transition:"all .2s",textAlign:"left",boxShadow:sel?`0 2px 12px ${lv.col}1e`:"none",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}><span style={{fontSize:"18px"}}>{lv.emoji}</span><div style={{flex:1}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"14px",color:sel?lv.col:T().INK}}>{lv.label}</div><div style={{fontSize:"11px",color:T().MUTE,marginTop:"1px"}}>{lv.sub}</div></div><span style={{fontSize:"11px",fontWeight:700,color:sel?lv.col:T().MUTE,padding:"2px 7px",borderRadius:"6px",background:sel?`${lv.col}10`:"transparent"}}>×{lv.mult}</span></button>);})}</div></div>}

        {/* Grid size */}
        <div style={{marginBottom:"20px"}}><div style={{fontSize:"11px",color:T().MUTE,letterSpacing:"1px",marginBottom:"10px",textTransform:"uppercase",fontWeight:600}}>Grille</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>{[{size:3,label:"3 × 3",sub:"3 en ligne",badge:"⚡"},{size:4,label:"4 × 4",sub:"4 en ligne",badge:"🧠"}].map(g=>{const sel=gridSize===g.size;return(<button key={g.size} onClick={()=>{sfx.tap();setGridSize(g.size);}} style={{padding:"14px",display:"flex",flexDirection:"column",gap:"6px",background:sel?T().CARD:"transparent",border:`1.5px solid ${sel?T().INK:T().BORDER}`,borderRadius:"14px",cursor:"pointer",transition:"all .2s",textAlign:"left",boxShadow:sel?"0 2px 12px rgba(0,0,0,0.1)":"none",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"18px",color:sel?T().INK:T().MUTE}}>{g.label}</div><span style={{fontSize:"16px"}}>{g.badge}</span></div><div style={{fontSize:"12px",color:T().MUTE}}>{g.sub}</div><div style={{display:"grid",gridTemplateColumns:`repeat(${g.size},1fr)`,gap:"2px",marginTop:"4px"}}>{Array.from({length:g.size*g.size}).map((_,i)=><div key={i} style={{aspectRatio:"1",borderRadius:"3px",background:sel?"#e7e5e4":T().CHIP2}}/>)}</div></button>);})}</div></div>

        {/* Rules */}
        <div style={{marginBottom:"28px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px"}}><div style={{fontSize:"11px",color:T().MUTE,letterSpacing:"1px",textTransform:"uppercase",fontWeight:600}}>Règles</div><div style={{fontSize:"12px",color:T().MUTE}}>{ac} active{ac>1?"s":""}</div></div>
          <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
...
export default function App(){
  ...
  // Update symOf proxy with current profiles
  _symOf_proxy.fn=(p)=>{
  ...
  const showToast=useCallback((msg,color=T().INK)=>{clearTimeout(toastRef.current);setToast({msg,color});toastRef.current=setTimeout(()=>setToast(null),2200);},[]);
  ...
  if(screen==="loading")return(<div style={{background:T().BG,minHeight:"100dvh",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:"24px",color:T().MUTE}}>Chargement…</div></div>);
    ...
    <div style={{background:T().BG,minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",paddingBottom:"28px",transition:"background .3s"}}>
      ...
      <div style={{width:"100%",maxWidth:"440px",padding:"18px 20px 0",display:"flex",alignItems:"center",gap:"12px"}}>
        ...
        <div style={{flex:1,textAlign:"center"}}>
          <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"20px",color:T().INK,letterSpacing:"-.5px"}}>Tic <span style={{color:XC}}>·</span> Tac <span style={{color:OC}}>·</span> Toe <span style={{fontSize:"13px",fontWeight:600,color:T().MUTE}}>{SIZE}×{SIZE}</span></div>
      ...
      <div style={{width:"100%",maxWidth:"440px",padding:"14px 20px 0",display:"flex",flexDirection:"column",gap:"10px"}}>
        {/* Blitz */}
        {blitz&&!cdActive&&<div style={{background:T().CARD,borderRadius:"16px",padding:"14px 16px",border:`1px solid ${txUrgent||toUrgent?`${XC}44`:T().BORDER}`,boxShadow:`0 1px 4px rgba(0,0,0,0.06)${txUrgent||toUrgent?`,0 0 0 3px ${XC}12`:""}`,transition:"all .3s"}}>
          <div style={{display:"flex",gap:"16px",alignItems:"center"}}>
            ...
            <div style={{width:"1px",height:"48px",background:T().BORDER,flexShrink:0}}/>
          </div>
        </div>}
        ...
        <div style={{display:"flex",gap:"8px"}}>
          {["X","O"].map(p=>{
            ...
            return(<div key={p} style={{flex:1,background:T().CARD,borderRadius:"16px",padding:"12px 14px",border:`1.5px solid ${active?col:T().BORDER}`,boxShadow:active?`0 0 0 3px ${col}18,0 2px 8px rgba(0,0,0,0.06)`:"0 1px 4px rgba(0,0,0,0.06)",transition:"all .2s",position:"relative",overflow:"hidden",transform:popping?"scale(1.04)":"scale(1)"}}>
          ...
          <div style={{background:T().CARD,borderRadius:"16px",padding:"12px",border:`1px solid ${T().BORDER}`,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"2px"}}>
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"22px",color:T().MUTE}}>{scores.draws}</div>
            <div style={{fontSize:"9px",color:T().MUTE,letterSpacing:"1px",textTransform:"uppercase"}}>Nuls</div>
          </div>
        </div>

        {/* Status */}
        <div style={{background:T().CARD,borderRadius:"12px",padding:"12px 16px",border:`1px solid ${powerMode?"#7c3aed44":T().BORDER}`,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",textAlign:"center",transition:"border-color .2s"}}>
