...
const BG="#fafaf9",CARD="#ffffff",INK="#0c0a09",MUTE="#78716c",BORDER="#e7e5e4";
...
const defProfile=(name,emoji)=>({name,emoji,points:0,wins:0,losses:0,draws:0,streak:0,bestStreak:0,totalGames:0,wonRounds:0});
...
function Toast({message,color}){if(!message)return null;return(<div style={{position:"fixed",top:"20px",left:"50%",transform:"translateX(-50%)",background:CARD,border:`1.5px solid ${color||BORDER}`,borderRadius:"24px",padding:"10px 20px",zIndex:190,boxShadow:`0 4px 20px ${color||"#000"}22`,animation:"toastIn .3s cubic-bezier(.34,1.56,.64,1)",display:"flex",alignItems:"center",gap:"8px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"14px",color:color||INK,whiteSpace:"nowrap",pointerEvents:"none"}}>{message}</div>);}
...
function VanishChoiceModal({squares,queue,player,shielded,onChoose,sfx}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(12,10,9,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:160,padding:"20px",backdropFilter:"blur(8px)",animation:"fadeIn .15s ease"}}>
      <div style={{background:CARD,borderRadius:"20px",padding:"24px 20px",width:"100%",maxWidth:"320px",boxShadow:"0 20px 60px rgba(0,0,0,0.18)",animation:"slideUp .25s cubic-bezier(.34,1.56,.64,1)"}}>
        <div style={{textAlign:"center",marginBottom:"16px"}}>
          ...
          <div style={{fontSize:"12px",color:MUTE,marginTop:"4px"}}>Vous avez trop de pièces — choisissez laquelle disparaît</div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
          {choices.map((idx,n)=>(
            <button key={idx} onClick={()=>{sfx.choose();onChoose(idx);}} style={{padding:"12px 16px",display:"flex",alignItems:"center",gap:"12px",background:"#f9f9f8",border:`1.5px solid ${BORDER}`,borderRadius:"12px",cursor:"pointer",transition:"all .15s",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>
              ...
              <div style={{flex:1,textAlign:"left"}}>
                <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:"13px",color:INK}}>Pièce {n+1}</div>
                <div style={{fontSize:"11px",color:MUTE}}>Case {idx+1} · {n===0?"La plus ancienne":"Récente"}</div>
              </div>
              <span style={{fontSize:"16px",color:MUTE}}>→</span>
            </button>
          ))}
        </div>
      </div>
...
function PointsOverlay({data,onClose}){
  if(!data)return null;
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(12,10,9,0.5)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:160,padding:"20px",backdropFilter:"blur(8px)",animation:"fadeIn .2s ease"}} onClick={onClose}>
      <div style={{background:CARD,borderRadius:"20px",padding:"28px",width:"100%",maxWidth:"300px",boxShadow:"0 20px 60px rgba(0,0,0,0.15)",animation:"slideUp .3s cubic-bezier(.34,1.56,.64,1)",textAlign:"center"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:"14px",color:MUTE,marginBottom:"16px"}}>{data.isLoss?"points perdus":"points gagnés !"}</div>
        <div style={{display:"flex",flexDirection:"column",gap:"6px",marginBottom:"20px"}}>
          {data.breakdown.map((b,i)=>(
            <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 12px",background:"#f9f9f8",borderRadius:"8px"}}>
...
function Stepper({value,min,max,onChange,label,unit,color}){const c=color||XC;return(<div style={{display:"flex",alignItems:"center",gap:"8px",justifyContent:"space-between",width:"100%"}}><span style={{fontSize:"13px",color:MUTE,flex:1}}>{label}</span><div style={{display:"flex",alignItems:"center",gap:"8px"}}><button onClick={()=>onChange(Math.max(min,value-1))} style={{width:"30px",height:"30px",borderRadius:"8px",background:"#f5f5f4",border:`1px solid ${BORDER}`,color:INK,fontSize:"17px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>−</button><span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"16px",color:c,minWidth:"38px",textAlign:"center"}}>{value}{unit}</span><button onClick={()=>onChange(Math.min(max,value+1))} style={{width:"30px",height:"30px",borderRadius:"8px",background:"#f5f5f4",border:`1px solid ${BORDER}`,color:INK,fontSize:"17px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>+</button></div></div>);}

function RuleCard({icon,title,sub,color,active,onToggle,children,badge}){const c=color||XC;return(<div style={{background:active?`${c}06`:"#f9f9f8",border:`1.5px solid ${active?c+"40":BORDER}`,borderRadius:"14px",overflow:"hidden",transition:"all .2s"}}><div style={{display:"flex",alignItems:"center",gap:"12px",padding:"14px 16px",cursor:"pointer",WebkitTapHighlightColor:"transparent"}} onClick={onToggle}><div style={{width:"38px",height:"38px",borderRadius:"10px",background:active?`${c}15`:"#f0efee",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"18px",flexShrink:0,transition:"background .2s"}}>{icon}</div><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:"6px"}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:"15px",color:active?c:INK,transition:"color .2s"}}>{title}</div>{badge&&<span style={{fontSize:"10px",fontWeight:700,padding:"1px 6px",borderRadius:"6px",background:`${c}15`,color:c}}>{badge}</span>}</div><div style={{fontSize:"12px",color:MUTE,marginTop:"1px"}}>{sub}</div></div><Toggle value={active} onChange={()=>{}} color={c}/></div>{active&&children&&<div style={{padding:"0 16px 14px 66px",display:"flex",flexDirection:"column",gap:"8px"}}>{children}</div>}</div>);}
...
function ProfileCard({profile,color,onEdit,showStats}){
  ...
  return(<div style={{background:CARD,borderRadius:"16px",padding:"14px 16px",border:`1.5px solid ${color}22`,boxShadow:`0 2px 12px ${color}14`}}><div style={{display:"flex",alignItems:"center",gap:"12px"}}><div style={{width:"44px",height:"44px",borderRadius:"50%",background:`${color}15`,border:`2px solid ${color}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"22px",flexShrink:0}}>{profile.emoji}</div><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"2px"}}><span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"15px",color:INK}}>{profile.name}</span><span style={{fontSize:"12px"}}>{rank.emoji}</span>{profile.streak>1&&<span style={{fontSize:"11px",fontWeight:700,color:"#f97316",background:"#fff7ed",padding:"1px 6px",borderRadius:"6px"}}>🔥{profile.streak}</span>}</div><div style={{display:"flex",gap:"6px",alignItems:"center"}}><span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"14px",color:GOLD}}>{profile.points}</span><span style={{fontSize:"11px",color:MUTE}}>pts · {rank.label}</span></div></div>{onEdit&&<button onClick={onEdit} style={{padding:"6px 12px",fontSize:"12px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,background:"#f5f5f4",border:`1px solid ${BORDER}`,color:MUTE,cursor:"pointer",borderRadius:"8px",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>✎</button>}</div>{showStats&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px",marginTop:"12px"}}>{[{l:"Victoires",v:profile.wins,c:"#22c55e"},{l:"Défaites",v:profile.losses,c:XC},{l:"Nuls",v:profile.draws,c:GOLD}].map(s=><div key={s.l} style={{background:"#f9f9f8",borderRadius:"8px",padding:"8px",textAlign:"center"}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"18px",color:s.c}}>{s.v}</div><div style={{fontSize:"10px",color:MUTE,marginTop:"1px"}}>{s.l}</div></div>)}</div>}</div>);
}

function ProfileEditor({initial,onSave,onBack}){
  const[name,setName]=useState(initial.name);const[emoji,setEmoji]=useState(initial.emoji);
  return(<div style={{background:BG,minHeight:"100dvh",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}><div style={{background:CARD,borderRadius:"20px",padding:"28px 24px",width:"100%",maxWidth:"360px",boxShadow:"0 8px 40px rgba(0,0,0,0.1)"}}><button onClick={onBack} style={{marginBottom:"16px",fontSize:"13px",color:MUTE,background:"none",border:"none",cursor:"pointer",padding:0}}>← Retour</button><div style={{display:"flex",flexDirection:"column",gap:"16px"}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"18px",color:INK,textAlign:"center"}}>Modifier le profil</div><div><div style={{fontSize:"12px",color:MUTE,marginBottom:"6px"}}>Pseudo</div><input value={name} onChange={e=>setName(e.target.value.slice(0,16))} placeholder="Votre pseudo..." style={{width:"100%",padding:"12px 14px",fontSize:"15px",fontFamily:"'Plus Jakarta Sans',sans-serif",border:`1.5px solid ${BORDER}`,borderRadius:"12px",outline:"none",color:INK,background:BG,boxSizing:"border-box"}}/></div><div><div style={{fontSize:"12px",color:MUTE,marginBottom:"8px"}}>Avatar</div><div style={{display:"grid",gridTemplateColumns:"repeat(8,1fr)",gap:"6px"}}>{EMOJIS.map(e=><button key={e} onClick={()=>setEmoji(e)} style={{aspectRatio:"1",fontSize:"20px",background:emoji===e?"#f0f0f0":BG,border:`1.5px solid ${emoji===e?INK:BORDER}`,borderRadius:"8px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>{e}</button>)}</div></div><button onClick={()=>onSave({...initial,name:name||initial.name,emoji})} disabled={!name.trim()} style={{padding:"15px",fontSize:"15px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,background:XG,border:"none",color:"#fff",cursor:"pointer",borderRadius:"14px",opacity:name.trim()?1:0.5,WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>Sauvegarder →</button></div></div></div>);
}

/* ── Options Panel ───────────────────────────────────────── */
function OptionsPanel({opts,onChange,onClose}){
  return(<div style={{position:"fixed",inset:0,background:"rgba(12,10,9,0.5)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:170,backdropFilter:"blur(8px)",animation:"fadeIn .2s ease"}} onClick={onClose}><div style={{background:CARD,borderRadius:"24px 24px 0 0",padding:"24px 20px 36px",width:"100%",maxWidth:"440px",boxShadow:"0 -8px 40px rgba(0,0,0,0.12)",animation:"slideUpSheet .3s ease"}} onClick={e=>e.stopPropagation()}><div style={{width:"36px",height:"4px",borderRadius:"2px",background:BORDER,margin:"0 auto 20px"}}/><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"18px",color:INK,marginBottom:"18px"}}>⚙️ Options</div><div style={{display:"flex",flexDirection:"column",gap:"14px"}}>{[{key:"sound",icon:"🔊",label:"Sons",sub:"Effets sonores"},{key:"animations",icon:"✨",label:"Animations",sub:"Confettis et effets"},{key:"showPoints",icon:"🏅",label:"Afficher les points",sub:"Overlay après chaque manche"},{key:"hoverPreview",icon:"👆",label:"Aperçu au survol",sub:"Voir votre symbole avant de jouer"},].map(o=><div key={o.key} style={{display:"flex",alignItems:"center",gap:"12px",cursor:"pointer",WebkitTapHighlightColor:"transparent"}} onClick={()=>onChange({...opts,[o.key]:!opts[o.key]})}><div style={{width:"38px",height:"38px",borderRadius:"10px",background:"#f5f5f4",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"18px",flexShrink:0}}>{o.icon}</div><div style={{flex:1}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,fontSize:"15px",color:INK}}>{o.label}</div><div style={{fontSize:"12px",color:MUTE,marginTop:"1px"}}>{o.sub}</div></div><Toggle value={opts[o.key]!==false} onChange={()=>onChange({...opts,[o.key]:!opts[o.key]})}/></div>)}</div><button onClick={onClose} style={{width:"100%",marginTop:"20px",padding:"14px",fontSize:"14px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,background:"#f5f5f4",border:"none",color:MUTE,cursor:"pointer",borderRadius:"12px",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>Fermer</button></div></div>);
}
...
function MenuScreen({profiles,onUpdateProfile,onStart,opts,onOpenOpts,sfx}){
  ...
  return(
    <div style={{background:BG,minHeight:"100dvh",overflowY:"auto"}}>
      <div style={{maxWidth:"400px",margin:"0 auto",padding:"32px 20px 52px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"28px"}}>
          <h1 style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"clamp(24px,7vw,36px)",color:INK,letterSpacing:"-1.5px",margin:0,lineHeight:1}}>Tic <span style={{color:XC}}>·</span> Tac <span style={{color:OC}}>·</span> Toe</h1>
          <button onClick={onOpenOpts} style={{width:"38px",height:"38px",borderRadius:"11px",background:CARD,border:`1px solid ${BORDER}`,color:MUTE,fontSize:"18px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 1px 3px rgba(0,0,0,0.06)",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>⚙️</button>
        </div>

        {/* Mode tabs */}
        <div style={{display:"flex",gap:"6px",marginBottom:"20px",background:"#f0efee",borderRadius:"14px",padding:"4px"}}>
        ...
        <div style={{marginBottom:"20px"}}>
          <div style={{fontSize:"11px",color:MUTE,letterSpacing:"1px",marginBottom:"10px",textTransform:"uppercase",fontWeight:600}}>Joueurs</div>
          <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
            {(tab==="2p"?["X","O"]:["X"]).map(p=><ProfileCard key={p} profile={profiles[p]} color={colOf(p)} onEdit={()=>setEditingP(p)} showStats={true}/>)}
            {tab==="ai"&&<div style={{background:CARD,borderRadius:"16px",padding:"12px 16px",border:`1.5px solid ${OC}22`}}><div style={{display:"flex",alignItems:"center",gap:"10px"}}><div style={{width:"44px",height:"44px",borderRadius:"50%",background:`${OC}15`,border:`2px solid ${OC}33`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"22px"}}>🤖</div><div><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"15px",color:INK}}>IA</div><div style={{fontSize:"12px",color:MUTE}}>{DIFFS.find(d=>d.id===aiLevel)?.label}</div></div></div></div>}
          </div>
        </div>

        {/* AI difficulty */}
        {tab==="ai"&&<div style={{marginBottom:"20px"}}><div style={{fontSize:"11px",color:MUTE,letterSpacing:"1px",marginBottom:"10px",textTransform:"uppercase",fontWeight:600}}>Difficulté</div><div style={{display:"flex",flexDirection:"column",gap:"6px"}}>{DIFFS.map(lv=>{const sel=aiLevel===lv.id;return(<button key={lv.id} onClick={()=>{sfx.tap();setAiLevel(lv.id);}} style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:"12px",background:sel?CARD:"transparent",border:`1.5px solid ${sel?lv.col:BORDER}`,borderRadius:"12px",cursor:"pointer",transition:"all .2s",textAlign:"left",boxShadow:sel?`0 2px 12px ${lv.col}1e`:"none",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}><span style={{fontSize:"18px"}}>{lv.emoji}</span><div style={{flex:1}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"14px",color:sel?lv.col:INK}}>{lv.label}</div><div style={{fontSize:"11px",color:MUTE,marginTop:"1px"}}>{lv.sub}</div></div><span style={{fontSize:"11px",fontWeight:700,color:sel?lv.col:MUTE,padding:"2px 7px",borderRadius:"6px",background:sel?`${lv.col}10`:"transparent"}}>×{lv.mult}</span></button>);})}</div></div>}

        {/* Grid size */}
        <div style={{marginBottom:"20px"}}><div style={{fontSize:"11px",color:MUTE,letterSpacing:"1px",marginBottom:"10px",textTransform:"uppercase",fontWeight:600}}>Grille</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px"}}>{[{size:3,label:"3 × 3",sub:"3 en ligne",badge:"⚡"},{size:4,label:"4 × 4",sub:"4 en ligne",badge:"🧠"}].map(g=>{const sel=gridSize===g.size;return(<button key={g.size} onClick={()=>{sfx.tap();setGridSize(g.size);}} style={{padding:"14px",display:"flex",flexDirection:"column",gap:"6px",background:sel?CARD:"transparent",border:`1.5px solid ${sel?INK:BORDER}`,borderRadius:"14px",cursor:"pointer",transition:"all .2s",textAlign:"left",boxShadow:sel?"0 2px 12px rgba(0,0,0,0.1)":"none",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"18px",color:sel?INK:MUTE}}>{g.label}</div><span style={{fontSize:"16px"}}>{g.badge}</span></div><div style={{fontSize:"12px",color:MUTE}}>{g.sub}</div><div style={{display:"grid",gridTemplateColumns:`repeat(${g.size},1fr)`,gap:"2px",marginTop:"4px"}}>{Array.from({length:g.size*g.size}).map((_,i)=><div key={i} style={{aspectRatio:"1",borderRadius:"3px",background:sel?"#e7e5e4":"#f0efee"}}/>)}</div></button>);})}</div></div>

        {/* Rules */}
        <div style={{marginBottom:"28px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"12px"}}><div style={{fontSize:"11px",color:MUTE,letterSpacing:"1px",textTransform:"uppercase",fontWeight:600}}>Règles</div><div style={{fontSize:"12px",color:MUTE}}>{ac} active{ac>1?"s":""}</div></div>
          <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
            ...
            <RuleCard icon="🌀" title="Rotation" color="#f472b6" active={rotateOn}
              ...
              <div style={{display:"flex",alignItems:"center",gap:"12px",cursor:"pointer",WebkitTapHighlightColor:"transparent"}} onClick={()=>{sfx.tap();setRotSurprise(!rotSurprise);}}>
                <span style={{fontSize:"13px",color:MUTE,flex:1}}>Mode surprise 🎲</span>
            ...
            <RuleCard icon="🏆" title="Best of" color={GOLD} active={bestOfOn}
              ...
              <div style={{display:"flex",alignItems:"center",gap:"12px",cursor:"pointer",WebkitTapHighlightColor:"transparent"}} onClick={()=>{sfx.tap();setDrawsDontCount(!drawsDontCount);}}>
                <span style={{fontSize:"13px",color:MUTE,flex:1}}>Les nuls ne comptent pas</span>
...
export default function App(){
  ...
  const[profiles,setProfiles]=useState({X:defProfile("Joueur 1","😎"),O:defProfile("Joueur 2","🦊")});
  const[opts,setOpts]=useState({sound:true,animations:true,showPoints:true,hoverPreview:true});
  ...
  const scored=useRef(false);const blitzRef=useRef(null);const toastRef=useRef(null);const moveRef=useRef(0);
  ...
  const awardPoints=useCallback(async(winner,loser,draw,movesPlayed,blitzLeft)=>{
    ...
    if(isAI){
      if(winner==="X"){
        ...
        await updateProfile("X",nX);
        if(extraCharge>0){setBombCharges(b=>({...b,X:b.X+1}));showToast("💣 Bombe rechargée ! (+1 charge)","#f97316");}
      ...
      if(extraW>0){setBombCharges(b=>({...b,[winner]:b[winner]+1}));showToast(`💣 ${profiles[winner].name} gagne une bombe !`,"#f97316");}
      if(opts.showPoints!==false)setPointsData(winPts);sfx.points();
    }
  ...
  useEffect(()=>{
    ...
    else if(!vanish&&squares.every(Boolean)){scored.current=true;clearInterval(blitzRef.current);setDraw(true);setScores(s=>({...s,draws:s.draws+1}));setFlash("draw");setTimeout(()=>setOverlay(true),580);sfx.draw();awardPoints(null,null,true,moveCount,0);}
  },[squares]);
  ...
  const hardReset=useCallback((bt)=>{
    ...
    clearInterval(blitzRef.current);setTimerX(t);setTimerO(t);setMoveCount(0);setShielded({});
    ...
    // New surprise trigger for this round
  ...
  const startGame=(mode,newCfg)=>{
    ...
    setStack([]);scored.current=false;setHover(null);setAiThinking(false);setVanishAnim(null);
    clearInterval(blitzRef.current);setTimerX(t);setTimerO(t);setMoveCount(0);moveRef.current=0;setShielded({});
  ...
  const doSymbolSwap=useCallback((sq,xQ,oQ)=>{
    const ns=sq.map(v=>v==="X"?"O":v==="O"?"X":null);
  ...
  const finalizeMove=useCallback((idx,sq,xQ,oQ,player,chosenRemove)=>{
    ...
    if(needsChoice){
      if(!isAI){
        ...
        setVanishChoice({player,pendingIdx:idx,pendingQueue:tempQ});
        sfx.choose();
        return;
      } else {
    ...
    moveRef.current+=1;const nm=moveRef.current;setMoveCount(nm);

    // Symbol swap at mid-game
    const swapAt=Math.floor((SIZE*SIZE)/2);
    if(swapOn&&!swapDone&&nm===swapAt){
      setTimeout(()=>doSymbolSwap(ns,nx,no),400);
    }

    // Rotation: normal or surprise
    const shouldRotate=rotateOn&&(rotSurprise?nm===surpriseTrigger:nm>0&&nm%ROT_EVR===0);
  ...
  const confirmVanishChoice=useCallback((chosenIdx)=>{
    ...
    const swapAt=Math.floor((SIZE*SIZE)/2);
    if(swapOn&&!swapDone&&nm===swapAt)setTimeout(()=>doSymbolSwap(ns,nx,no),400);
    const shouldRotate=rotateOn&&(rotSurprise?nm===surpriseTrigger:nm>0&&nm%ROT_EVR===0);
    if(shouldRotate)setTimeout(()=>doRotate(ns,nx,no,shieldedCells),swapOn&&!swapDone&&nm===swapAt?900:100);
  ...
  if(screen==="loading")return(<div style={{background:BG,minHeight:"100dvh",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontSize:"24px",color:MUTE}}>Chargement…</div></div>);
  if(screen==="menu")return(<div style={{background:BG,minHeight:"100dvh"}}><style>{ANIM}</style>{showOpts&&<OptionsPanel opts={opts} onChange={saveOpts} onClose={()=>setShowOpts(false)}/>}<MenuScreen profiles={profiles} onUpdateProfile={updateProfile} onStart={startGame} opts={opts} onOpenOpts={()=>setShowOpts(true)} sfx={sfx}/></div>);

  return(
    <div style={{background:BG,minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",paddingBottom:"28px"}}>
      <style>{ANIM}</style>
      ...
      <div style={{width:"100%",maxWidth:"440px",padding:"18px 20px 0",display:"flex",alignItems:"center",gap:"12px"}}>
        <button onClick={goMenu} style={{width:"38px",height:"38px",borderRadius:"11px",background:CARD,border:`1px solid ${BORDER}`,color:MUTE,fontSize:"17px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>←</button>
        <div style={{flex:1,textAlign:"center"}}>
          <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"20px",color:INK,letterSpacing:"-.5px"}}>Tic <span style={{color:XC}}>·</span> Tac <span style={{color:OC}}>·</span> Toe <span style={{fontSize:"13px",fontWeight:600,color:MUTE}}>{SIZE}×{SIZE}</span></div>
          <div style={{fontSize:"11px",color:MUTE,marginTop:"2px",display:"flex",gap:"6px",justifyContent:"center",flexWrap:"wrap"}}>
            <span>M.{round}</span>
        ...
        <button onClick={()=>setShowOpts(true)} style={{width:"38px",height:"38px",borderRadius:"11px",background:CARD,border:`1px solid ${BORDER}`,color:MUTE,fontSize:"17px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>⚙️</button>
      </div>

      <div style={{width:"100%",maxWidth:"440px",padding:"14px 20px 0",display:"flex",flexDirection:"column",gap:"10px"}}>
        {/* Blitz */}
        {blitz&&!cdActive&&<div style={{background:CARD,borderRadius:"16px",padding:"14px 16px",border:`1px solid ${txUrgent||toUrgent?`${XC}44`:BORDER}`,boxShadow:`0 1px 4px rgba(0,0,0,0.06)${txUrgent||toUrgent?`,0 0 0 3px ${XC}12`:""}`,transition:"all .3s"}}>
          <div style={{display:"flex",gap:"16px",alignItems:"center"}}>
            {[{p:"X",t:timerX,urg:txUrgent,act:xIsNext},{p:"O",t:timerO,urg:toUrgent,act:!xIsNext}].map(({p,t,urg,act})=>(
              <div key={p} style={{flex:1,opacity:!act&&!isOver?0.4:1,transition:"opacity .3s"}}>
                ...
                <div style={{height:"4px",background:"#f0efee",borderRadius:"2px",overflow:"hidden",transform:p==="O"?"scaleX(-1)":"none"}}><div style={{height:"100%",borderRadius:"2px",background:urg?XG:gradOf(p),width:`${(t/blitzTime)*100}%`,transition:"width .5s ease"}}/></div>
                <div style={{fontSize:"11px",color:MUTE,marginTop:"4px",textAlign:p==="O"?"right":"left"}}>{aiThinking&&p==="O"&&isAI?"Réfléchit…":profiles[p].name}</div>
              </div>
            ))}
        ...
        <div style={{display:"flex",gap:"8px"}}>
          {["X","O"].map(p=>{
            ...
            return(<div key={p} style={{flex:1,background:CARD,borderRadius:"16px",padding:"12px 14px",border:`1.5px solid ${active?col:BORDER}`,boxShadow:active?`0 0 0 3px ${col}18,0 2px 8px rgba(0,0,0,0.06)`:"0 1px 4px rgba(0,0,0,0.06)",transition:"all .2s",position:"relative",overflow:"hidden",transform:popping?"scale(1.04)":"scale(1)"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                <div style={{width:"36px",height:"36px",borderRadius:"50%",background:`${col}15`,border:`1.5px solid ${col}30`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"18px"}}>{isAiP?"🤖":prof.emoji}</div>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:"4px"}}>
                    <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:700,fontSize:"12px",color:INK,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"70px"}}>{isAiP?`IA ${aiConf.emoji}`:prof.name}</span>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:"4px",marginTop:"1px"}}>
                    <span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"22px",color:INK,lineHeight:1}}>{scores[p]}</span>
          ...
          <div style={{background:CARD,borderRadius:"16px",padding:"12px",border:`1px solid ${BORDER}`,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"2px"}}>
            <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:"22px",color:MUTE}}>{scores.draws}</div>
            <div style={{fontSize:"9px",color:MUTE,letterSpacing:"1px",textTransform:"uppercase"}}>Nuls</div>
          </div>
        </div>

        {/* Status */}
        <div style={{background:CARD,borderRadius:"12px",padding:"12px 16px",border:`1px solid ${powerMode?"#7c3aed44":BORDER}`,boxShadow:"0 1px 4px rgba(0,0,0,0.06)",textAlign:"center",transition:"border-color .2s"}}>
        ...
        {/* Board */}
        <div style={{background:CARD,borderRadius:"22px",padding:"8px",boxShadow:"0 2px 16px rgba(0,0,0,0.08),0 0 0 1px rgba(0,0,0,0.04)",transform:rotAnim?"rotate(90deg)":"rotate(0deg)",transition:rotAnim?"transform .45s cubic-bezier(.4,0,.2,1)":"none",position:"relative"}}>
          <div style={{display:"grid",gridTemplateColumns:`repeat(${SIZE},${cellSize})`,gridTemplateRows:`repeat(${SIZE},${cellSize})`,position:"relative"}} onMouseLeave={()=>setHover(null)}>
            {squares.map((val,i)=>{
              ...
              return(<div key={i} onClick={()=>handleCell(i)} onMouseEnter={()=>setHover(i)}
                ...
                {val&&<span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:symSize,display:"block",lineHeight:1,color:col,opacity:isVanishing?0.05:(isNVX||isNVO)?0.35:1,transform:isVanishing?"scale(0.3) rotate(180deg)":"scale(1)",transition:isVanishing?"all .4s ease":"none",animation:ripple===i?"popIn .2s cubic-bezier(.34,1.56,.64,1) both":"none"}}>{val}</span>}
                {!val&&hover===i&&canPlay&&opts.hoverPreview!==false&&<span style={{fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:800,fontSize:symSize,display:"block",lineHeight:1,color:colOf(cur),opacity:0.13,pointerEvents:"none"}}>{cur}</span>}
              </div>);
            })}
        ...
        {/* Actions */}
        <div style={{display:"flex",gap:"8px"}}>
          <button onClick={undo} disabled={!stack.length||!!winInfo||isDraw||aiThinking||!!powerMode||!!vanishChoice} style={{flex:1,padding:"14px",fontSize:"14px",fontFamily:"'Plus Jakarta Sans',sans-serif",fontWeight:600,background:CARD,border:`1px solid ${BORDER}`,color:MUTE,cursor:"pointer",borderRadius:"14px",transition:"all .15s",opacity:!stack.length||winInfo||isDraw||aiThinking||powerMode||vanishChoice?0.35:1,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",WebkitTapHighlightColor:"transparent",touchAction:"manipulation"}}>← Annuler</button>
...
const ANIM=`
  ...
  *,*::before,*::after{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
  html,body{overflow-x:hidden;touch-action:manipulation;background:${BG};margin:0;}
